...
DELIMITADOR;
    ...
    function get_productoLand() {
        if(!isset($_GET['id']) || empty($_GET['id'])) {
            ...
            $query = query("SELECT *, COUNT(b.com_prod_id) AS total_com, SUM(b.com_puntaje) AS suma FROM productos a LEFT JOIN comentarios b ON a.prod_id = b.com_prod_id WHERE prod_id = {$id} GROUP BY b.com_prod_id");
            if(contar_filas($query) == 0) {
                redirect("./");
            } else {
